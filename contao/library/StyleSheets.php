...
class StyleSheets extends Backend
{
	 ...
	 */
	public function compileDefinition($row, $blnWriteToFile=false, $vars=[], $parent=[])
	{
		if ($blnWriteToFile)
		{
			$strGlue = '../../';
		...
		if ($row['border'])
		{
			...
			if ($row['borderradius'] != '')
			{
				...
				if (is_array($row['borderradius']) && ($row['borderradius']['top'] != '' || $row['borderradius']['right'] != '' || $row['borderradius']['bottom'] != '' || $row['borderradius']['left'] != ''))
				{
					...
					{
						$arrDir = ['top-left'=>$top, 'top-right'=>$right, 'bottom-right'=>$bottom, 'bottom-left'=>$left];
	 ...
	 */
	protected function compileColor($color, $blnWriteToFile=false, $vars=[])
	{
		if (!is_array($color))
		{
	 ...
	 *
	 * @return string The shortened hex color
	 */
	 ...
	 */
	protected function convertHexColor($color, $blnWriteToFile=false, $vars=[])
	{
		...
		if (Input::post('FORM_SUBMIT') == 'tl_style_sheet_import')
		{
			...
			foreach ($arrUploaded as $strCssFile)
			{
				...
				$objStyleSheet = $this->Database->prepare("INSERT INTO tl_style_sheet (pid, tstamp, name, media) VALUES (?, ?, ?, ?)")
												->execute(Input::get('id'), time(), $strName, ['all']);
				...
				foreach ($arrTokens as $arrToken)
				{
					...
					if ($arrToken['type'] == 'comment')
					{
						...
						{
							$strCategory = trim(str_replace(['/*', '*/', '*'], '', $arrToken['content']));
						}

						// Declaration comment
						elseif (strpos($arrToken['content'], "\n") === false)
						{
							$strComment = trim(str_replace(['/*', '*/', '*'], '', $arrToken['content']));
						}
					}
...
<div class="tl_submit_container">
  <input type="submit" name="save" id="save" class="tl_submit" accesskey="s" value="' .specialchars($GLOBALS['TL_LANG']['tl_style_sheet']['import'][0]). '">
	...
	/**
	 * Add a suffix if a stlye sheet name exists
	 *
	 ...
	 *
	 * @param array $arrDefinition The data array
	 */
