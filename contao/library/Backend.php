...
abstract class Backend extends Controller
{
	 ...
	 *
	 * @return string The TinyMCE language code
	 */
	 ...
	 *
	 * @return string The ACE type string
	 */
	 ...
	 *
	 * @param string $strKey The session key
	 */
	public static function addPagesBreadcrumb($strKey='tl_page_node')
	{
		...
		if ($intNode)
		{
			...
			do
			{
				...
				{
					$arrLinks[] = self::addPageIcon($objPage->row(), '', null, '', true) . ' <a href="' . Controller::addToUrl('node='.$objPage->id) . '" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['selectNode']).'">' . $objPage->title . '</a>';
				}
  ...
  <li>' . implode(' &gt; </li><li>', $arrLinks) . '</li>
	...
	public static function addFilesBreadcrumb($strKey='tl_files_node')
	{
		...
		foreach ($arrNodes as $strFolder)
		{
			...
			{
				$arrLinks[] = '<img src="' . TL_FILES_URL . 'system/themes/' . self::getTheme() . '/images/folderC.gif" width="18" height="18" alt=""> ' . $strFolder;
			}
			else
			{
				$arrLinks[] = '<img src="' . TL_FILES_URL . 'system/themes/' . self::getTheme() . '/images/folderC.gif" width="18" height="18" alt=""> <a href="' . Controller::addToUrl('node='.$strPath) . '" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['selectNode']).'">' . $strFolder . '</a>';
			}
		}
