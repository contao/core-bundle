 ...
 * Usage:
 *
 *     $versions = new Versions('tl_content', 22);
 *     $versions->create();
 *
 *     $versions->restore(1);
 *
			...
			if ($intTo > 0 && $intFrom > 0)
			{
				...
				foreach ($to as $k=>$v)
				{
					if ($from[$k] != $to[$k])
					{
						...
						$strBuffer .= $objDiff->Render(new DiffRenderer(['field'=>($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['label'][0] ?: (isset($GLOBALS['TL_LANG']['MSC'][$k]) ? (is_array($GLOBALS['TL_LANG']['MSC'][$k]) ? $GLOBALS['TL_LANG']['MSC'][$k][0] : $GLOBALS['TL_LANG']['MSC'][$k]) : $k))]));
					}
				}
			}
		}
...
';
	 ...
	 *
	 * @param BackendTemplate $objTemplate The back end template object
	 */
