 ...
 */
class ArticleTree extends \Widget implements DcaFilterInterface
{
	...
	public function getDcaFilter()
	{
		...
		if (is_array($this->rootNodes))
		{
			...
			{
				$root = array_intersect(array_merge($this->rootNodes, $this->Database->getChildRecords($this->rootNodes, 'tl_article')), $GLOBALS['TL_DCA']['tl_page']['list']['sorting']['root']);
	...
	protected function checkValue($varInput)
	{
		...
		if (count(array_diff($arrIds, array_merge($this->rootNodes, $this->Database->getChildRecords($this->rootNodes, 'tl_article')))) > 0)
		{
		...
		if (!empty($this->varValue)) // Can be an array
		{
			...
			if ($objArticles !== null)
			{
				while ($objArticles->next())
				{
					...
					$arrValues[$objArticles->id] = \Image::getHtml($this->getPageStatusIcon($objArticles)) . ' ' . $objArticles->title . ' (' . $objArticles->alias . \Config::get('urlSuffix') . ')';
				}
			}
  ...
  <div class="selector_container">' . (($blnHasOrder && count($arrValues) > 1) ? '
    ...
    <ul id="sort_'.$this->strId.'" class="'.($blnHasOrder ? 'sortable' : '').'">';
		...
		$return .= '</ul>
    <p><a href="' . ampersand(\System::getContainer()->get('router')->generate('contao_backend_picker', array('do'=>'article', 'context'=>'article', 'target'=>$this->strTable.'.'.$this->strField.'.'.$this->activeRecord->id, 'value'=>implode(',', $arrSet), 'popup'=>1))) . '" class="tl_submit" id="pt_' . $this->strField . '">'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</a></p>
    <script>
